...
func main() {
	// 定义命令行参数 filename，默认值为 "server"
	filepath := flag.String("filepath", "file", "Name of the file to download")

	// 解析命令行参数
	flag.Parse()

	// // 拼接 URL
	// url := "http://localhost:8919/download?filename=" + *filename
	// savePath := "./"
...
func DownloadFileDir(fileName, savePath string) (string, error) {
	...
	// 5. 发送下载请求
	downloadURL := "http://120.220.95.189:8121/download?filename=" + fileName
