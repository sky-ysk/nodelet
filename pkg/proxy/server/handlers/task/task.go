...
func (h *TaskHandler) DeleteTask(request *restful.Request, response *restful.Response) {
	...
	// 查看task是否存在
	task, err := h.client.Get(context.TODO(), name, metav1.GetOptions{})
	if err != nil {
		logs.Errorf("Get task %s error: %v , task not exist!", name, err)
		err := response.WriteError(http.StatusNotFound, err)
		if err != nil {
...
func (h *TaskHandler) UpdateTask(request *restful.Request, response *restful.Response) {
	...
	if task.Name == name {
		...
		// 格式验证
		res, err := analyzer.SerializeToJson(task)
		_, err = analyzer.Deserialize(res, apis.Task{})
		if err != nil {
			err := response.WriteError(http.StatusBadRequest, err)
			if err != nil {
		...
		// 这个函数执行的时候返回错误
		updatedTask, updateErr := h.client.Update(context.TODO(), task, metav1.UpdateOptions{})
		if updateErr != nil {
...
func (h *TaskHandler) PatchTask(request *restful.Request, response *restful.Response) {
	...
	if task.Name == name {
		...
		patchedTask, err := h.client.Patch(context.TODO(), name, types.StrategicMergePatchType, []byte(patchTask), metav1.PatchOptions{})
		if err != nil {
