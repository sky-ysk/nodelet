...
func main() {
	...
	tasksClient := clientSet.Core().Tasks(apis.NamespaceDefault)
	...
	taskListWatcher := cache.NewListWatchFromClient(clientSet.Core().RESTClient(), "tasks", apis.NamespaceDefault, fields.Everything())
	...
	fmt.Println("1 task Updated task...")

	// Delete一个Task
	// 删除Task后，Indexer就查询不到结点了
	//fmt.Println("deleting")
	//err = tasksClient.Delete(context.TODO(), "demo-tasks", meta.DeleteOptions{})
	//if err != nil {
	//	panic(err)
	//}
...
func createMockAPIServer() *http.Server {
	...
	mux.HandleFunc("/apis/resources/v1/defaultNamespace/tasks", func(w http.ResponseWriter, r *http.Request) {
		query := r.URL.Query()
	...
	mux.HandleFunc("/apis/resources/v1/defaultNamespace/tasks/demo-tasks", func(w http.ResponseWriter, r *http.Request) {
