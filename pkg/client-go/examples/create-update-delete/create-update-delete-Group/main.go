...
func main() {
	...
	patchResult, err := groupsClient.Patch(context.TODO(), "demo-groups", types.StrategicMergePatchType, []byte(patchGroup), metav1.PatchOptions{})
...
func createMockAPIServer() *http.Server {
	...
	mux.HandleFunc("/apis/resources/v1/groups", func(w http.ResponseWriter, r *http.Request) {
		query := r.URL.Query()
		...
		case http.MethodGet:
			if iswatch == "true" { // 判断是否是Watch请求
				...
				groupList.Items = filteredGroups
				// 返回筛选后的节点列表
				w.Header().Set("Content-Type", "application/json; charset=UTF-8")
				w.WriteHeader(http.StatusOK)
	...
	mux.HandleFunc("/apis/resources/v1/groups/demo-groups", func(w http.ResponseWriter, r *http.Request) {
		...
		case http.MethodPatch: // PATCH 部分更新
			...
			// 应用 Patch 到现有的节点
			updatedGroup := group
			if err := json.Unmarshal(patchData, &updatedGroup); err != nil {
