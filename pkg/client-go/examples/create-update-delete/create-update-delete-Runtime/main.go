...
func main() {
	...
	_, err = runtimesClient.Create(context.TODO(), runtime3, metav1.CreateOptions{})
	prompt()
	...
	logs.Info("updating")
	// 部分更改一个参数
	// 先Get一个Runtime ,更改Runtime的参数, UpdateRuntime

	result, getErr := runtimesClient.Get(context.TODO(), "demo-runtimes", metav1.GetOptions{})
	if getErr != nil {
	...
	_, updateErr := runtimesClient.Update(context.TODO(), result, metav1.UpdateOptions{})
	if updateErr != nil {
		logs.Error(fmt.Errorf("Update failed: %v", updateErr))
	}

	logs.Infof("修改后的result.Spec.Name：", result.Spec.Name)
	logs.Info("Updated runtime...")
	prompt()

	// List 所有Runtime
	logs.Info("listing 筛选的runtime")
	...
	//Patch 一个Runtime
	logs.Info("patching")
	patchResult, err := runtimesClient.Patch(context.TODO(), "demo-runtimes", types.StrategicMergePatchType, patchRuntime, metav1.PatchOptions{})
	...
	// Delete一个Runtime
	logs.Info("deleting")
	err = runtimesClient.Delete(context.TODO(), "demo-runtimes", metav1.DeleteOptions{})
	if err != nil {
		panic(err)
	}
	logs.Info("Deleted runtime...")
	prompt()

	// Delete 之后再次 List所有Runtime
	logs.Info("listing")
	lstOpts = metav1.ListOptions{}
	list, err = runtimesClient.List(context.TODO(), lstOpts)
	if err != nil {
	...
	//DeleteCollection 删除所有Spec.Name=demo-runtime的Runtime
	logs.Info("deleting collection")
	lstOpts = metav1.ListOptions{
		FieldSelector: "Spec.Name=demo-runtime",
	}
	err = runtimesClient.DeleteCollection(context.TODO(), metav1.DeleteOptions{}, lstOpts)
	if err != nil {
		logs.Error(err)
	}
	...
	// DeleteCollection 之后再次 List所有Runtime
	logs.Info("listing")
	lstOpts = metav1.ListOptions{}
	list, err = runtimesClient.List(context.TODO(), lstOpts)
	if err != nil {
