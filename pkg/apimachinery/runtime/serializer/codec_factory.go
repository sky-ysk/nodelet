...
//	"hit.edu/framework/pkg/apimachinery/runtime/serializer/json"
//	"hit.edu/framework/pkg/apimachinery/runtime/serializer/protobuf"
//	"hit.edu/framework/pkg/apimachinery/runtime/serializer/recognizer"
//	"hit.edu/framework/pkg/apimachinery/runtime/versioning"
//	"mime"
//	"strings"
//)
//
...
func (f WithoutConversionCodecFactory) DecoderToVersion(serializer runtime.Decoder) runtime.Decoder {
	return serializer

}

// EncoderForVersion 返回一个指定版本的编码器， 这里我们不使用版本控制，所以直接返回编码器。
func (f CodecFactory) EncoderForVersion(encoder runtime.Encoder, _ runtime.GroupVersioner) runtime.Encoder {
	return encoder // 直接返回原始的编码器
}

// DecoderToVersion 返回一个指定版本的解码器，这里我们不使用版本控制，直接返回解码器。
func (f CodecFactory) DecoderToVersion(decoder runtime.Decoder, _ runtime.GroupVersioner) runtime.Decoder {
	return decoder // 直接返回原始的解码器
}
...
func NewCodecFactory(scheme *runtime.Scheme) *CodecFactory {
	...
	// 创建支持流式的 JSON 序列化器
	jsonStreamSerializer := &runtime.StreamSerializerInfo{
		EncodesAsText: true,
		Serializer:    jsonSerializer, // 使用相同的 JSON 序列化器
		Framer:        json.Framer,
	}

	// 创建并返回 CodecFactory，支持 application/json 媒体类型
	return &CodecFactory{
		accepts: []runtime.SerializerInfo{
			{
				MediaType:        "application/json", // 支持 JSON 媒体类型
				Serializer:       jsonSerializer,
				StreamSerializer: jsonStreamSerializer, // 添加流式序列化器
			},
		},
...
func (f CodecFactory) LegacyCodec() runtime.Codec {
	return f.legacySerializer
}
