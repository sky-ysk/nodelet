...
func (s *Scheme) ObjectKinds(obj Object) ([]schema.GroupVersionKind, bool, error) {
	...
	if _, ok := obj.(Unstructured); ok {
		...
		if len(gvk.Kind) == 0 {
			return nil, false, fmt.Errorf("unstructured object has no kind")
		}
		if len(gvk.Version) == 0 {
			return nil, false, fmt.Errorf("unstructured object has no version")
		}
	...
	if !ok {
		return nil, false, fmt.Errorf(s.schemeName, t)
	}
...
func (s *Scheme) New(kind schema.GroupVersionKind) (Object, error) {
	...
	return nil, fmt.Errorf(s.schemeName, kind)
}
...
func (s *Scheme) convertToVersion(copy bool, in Object, target GroupVersioner) (Object, error) {
	...
	if !ok || len(kinds) == 0 {
		return nil, fmt.Errorf(s.schemeName, t)
	}
	...
	if !ok {
		...
		return nil, fmt.Errorf(s.schemeName, t, target)
	}
...
var internalPackages = []string{"hit.edu/framework/pkg/apimachinery/runtime/scheme.go"}
