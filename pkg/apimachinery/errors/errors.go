...
func HasStatusCause(err error, name meta.CauseType) bool {
...
func StatusCause(err error, name meta.CauseType) (meta.StatusCause, bool) {
...
// FromObject generates an StatusError from an meta.Status, if that is the type of obj; otherwise,
...
func NewApplyConflict(causes []meta.StatusCause, message string) *StatusError {
	return &StatusError{ErrStatus: meta.Status{
		Status: meta.StatusFailure,
		Code:   http.StatusConflict,
		Reason: meta.StatusReasonConflict,
		Details: &meta.StatusDetails{
...
func NewResourceExpired(message string) *StatusError {
	return &StatusError{meta.Status{
		Status:  meta.StatusFailure,
		Code:    http.StatusGone,
		Reason:  meta.StatusReasonExpired,
		Message: message,
	}}
}
...
func NewInvalid(qualifiedKind schema.GroupKind, name string, errs field.ErrorList) *StatusError {
	causes := make([]meta.StatusCause, 0, len(errs))
	for i := range errs {
		err := errs[i]
		causes = append(causes, meta.StatusCause{
			Type:    meta.CauseType(err.Type),
...
func NewGenericServerResponse(code int, verb string, qualifiedResource schema.GroupResource, name, serverMessage string, retryAfterSeconds int, isUnexpectedResponse bool) *StatusError {
	...
	case http.StatusUnsupportedMediaType:
		reason = meta.StatusReasonUnsupportedMediaType
		// the server message has details about what types are acceptable
		message = serverMessage
	...
	var causes []meta.StatusCause
	if isUnexpectedResponse {
		causes = []meta.StatusCause{
			{
				Type:    meta.CauseTypeUnexpectedServerResponse,
				Message: serverMessage,
			},
		}
	} else {
...
func IsNotFound(err error) bool {
	...
	if reason == meta.StatusReasonNotFound {
		return true
	}
...
func IsConflict(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonConflict {
		return true
	}
...
func IsInvalid(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonInvalid {
		return true
	}
...
func IsGone(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonGone {
		return true
	}
...
func IsResourceExpired(err error) bool {
	return ReasonForError(err) == meta.StatusReasonExpired
}
...
func IsNotAcceptable(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonNotAcceptable {
		return true
	}
...
func IsUnsupportedMediaType(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonUnsupportedMediaType {
		return true
	}
...
func IsMethodNotSupported(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonMethodNotAllowed {
		return true
	}
...
func IsServiceUnavailable(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonServiceUnavailable {
		return true
	}
...
func IsBadRequest(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonBadRequest {
		return true
	}
...
func IsUnauthorized(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonUnauthorized {
		return true
	}
...
func IsForbidden(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonForbidden {
		return true
	}
...
func IsTimeout(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonTimeout {
		return true
	}
...
func IsServerTimeout(err error) bool {
	...
	return ReasonForError(err) == meta.StatusReasonServerTimeout
}
...
func IsInternalError(err error) bool {
	reason, code := reasonAndCodeForError(err)
	if reason == meta.StatusReasonInternalError {
		return true
	}
...
func IsUnexpectedServerError(err error) bool {
	...
	if (ok || errors.As(err, &status)) && status.Status().Details != nil {
		for _, cause := range status.Status().Details.Causes {
			if cause.Type == meta.CauseTypeUnexpectedServerResponse {
				return true
			}
		}
	}
...
func SuggestsClientDelay(err error) (int, bool) {
	...
	if (ok || errors.As(err, &t)) && t.Status().Details != nil {
		...
		case meta.StatusReasonServerTimeout:
...
func (r *ErrorReporter) AsObject(err error) runtime.Object {
	...
	if status.ErrStatus.Details == nil {
		status.ErrStatus.Details = &meta.StatusDetails{}
	}
	reason := r.reason
	if len(reason) == 0 {
		reason = "ClientError"
	}
	status.ErrStatus.Details.Causes = append(status.ErrStatus.Details.Causes, meta.StatusCause{
		Type:    meta.CauseType(reason),
		Message: err.Error(),
	})
