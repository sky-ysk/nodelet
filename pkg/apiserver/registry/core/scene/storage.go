...
func NewListFunc() runtime.Object {
	return &apis.SceneList{}
}

// GetAttrs 从传入的资源对象中提取标签和字段
func GetAttrs(obj runtime.Object) (labels.Set, fields.Set, error) {
	Scene, ok := obj.(*apis.Scene)
	if !ok {
		return nil, nil, fmt.Errorf("not a Scene")
	}
	return labels.Set(Scene.ObjectMeta.Labels), generic.ObjectMetaFieldsSet(&Scene.ObjectMeta, true), nil
}
...
func NewSceneStorage(optsGetter generic.RESTOptionsGetter) (SceneStorage, error) {

	store := &genericregistry.Store{
		NewFunc:                   NewFunc,
		NewListFunc:               NewListFunc,
		PredicateFunc:             Match,
		DefaultQualifiedResource:  apis.Resource("Scenes"),
		SingularQualifiedResource: apis.Resource("Scene"),
