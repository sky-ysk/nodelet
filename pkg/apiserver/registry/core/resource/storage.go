...
func NewListFunc() runtime.Object {
	return &apis.Resource_NodeList{}
}

// GetAttrs 从传入的资源对象中提取标签和字段
func GetAttrs(obj runtime.Object) (labels.Set, fields.Set, error) {
	Resource, ok := obj.(*apis.Resource_Node)
	if !ok {
		return nil, nil, fmt.Errorf("not a Resource_Node")
	}
	return labels.Set(Resource.ObjectMeta.Labels), generic.ObjectMetaFieldsSet(&Resource.ObjectMeta, true), nil
}
...
func NewResourceStorage(optsGetter generic.RESTOptionsGetter) (ResourceStorage, error) {

	store := &genericregistry.Store{
		NewFunc:                   NewFunc,
		NewListFunc:               NewListFunc,
		PredicateFunc:             Match,
		DefaultQualifiedResource:  apis.Resource("Resource_Nodes"),
		SingularQualifiedResource: apis.Resource("Resource_Node"),
	...
	Resource_NodeREST := &REST{Store: store}
	statusREST := &StatusREST{Store: &statusStore}
	specREST := &SpecREST{Store: &specStore}
	return ResourceStorage{
		Resource: Resource_NodeREST,
		Status:   statusREST,
		Spec:     specREST,
	}, nil
}
