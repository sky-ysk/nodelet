...
// DeepEqualSafeNodeSpec returns an apis.NodeSpec safe for deep-equal operations.
func DeepEqualSafeNodeSpec() apis.NodeSpec {
	return apis.NodeSpec{}
}
...
type sortableNodeList []apis.Node

func (s sortableNodeList) Len() int {
	return len(s)
}

func (s sortableNodeList) Less(i, j int) bool {
	return computeNodeKey(&s[i]) < computeNodeKey(&s[j])
}

func (s sortableNodeList) Swap(i, j int) {
	s[i], s[j] = s[j], s[i]
}
